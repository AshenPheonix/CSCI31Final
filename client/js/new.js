...
riot.tag2('create', '<form> <fieldset> <legend>Enter your Post here {opts.user}</legend> <div class="form-group"> <label for="title">Post Title</label> <input type="text" class="form-control" id="title" placeholder="Title"> </div> <div class="form-group"> <label for="body">Post Body</label> <textarea id="postbody"> </textarea> </div> <button type="button" class="col-md-4" id="subbut" onclick="{submit}">Create post</button> <span class="col-md-8" id="Error-Field" __hidden="{!error}"></span> </fieldset> </form>', 'create p,[riot-tag="create"] p,[data-is="create"] p{ color: #000; }', '', function(opts) {
    this.on("mount", () => {
      tinymce.init({
        selector:'#postbody'
      })
